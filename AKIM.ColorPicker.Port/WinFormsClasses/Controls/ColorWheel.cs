    ...
    public class ColorWheel : Control, IColorEditor
    {
        ...
        protected float GetRadius(PointF centerPoint)
        {
            ...
            return Math.Min(centerPoint.X, centerPoint.Y) -
                   (Math.Max(padding.Horizontal, padding.Vertical) + _selectionSize / 2);
        }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (this.AllowPainting)
            {
                ...
                if (this.BackgroundImage == null && parent != null &&
                    (this.BackColor == parent.BackColor || parent.BackColor.A != 255))
                {
                ...
                using (Pen pen = new Pen(this.BackColor, 2))
                {
                    e.Graphics.DrawEllipse(pen,
                        new RectangleF(_centerPoint.X - _radius, _centerPoint.Y - _radius, _radius * 2, _radius * 2));
                }
            ...
            if (!float.IsNaN(location.X) && !float.IsNaN(location.Y))
            {
                ...
                if (this.Focused && includeFocus)
                {
                    ControlPaint.DrawFocusRectangle(e.Graphics,
                        new Rectangle(x - 1, y - 1, _selectionSize + 2, _selectionSize + 2));
                }
            }
        }
